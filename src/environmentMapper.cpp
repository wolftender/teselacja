...
DirectX::XMMATRIX mini::gk2::EnvironmentMapper::FaceViewMtx (D3D11_TEXTURECUBE_FACE face) const {
	...
	eye = { m_position.x, m_position.y, m_position.z };

	// TODO: 1.15 Setup view matrix
	switch (face) {
	case D3D11_TEXTURECUBE_FACE_POSITIVE_X:
		target = { m_position.x + 1.0f, m_position.y, m_position.z };
		up = { 0.0f, 1.0f, 0.0f };
		break;
	...
	case D3D11_TEXTURECUBE_FACE_POSITIVE_Y:
		target = { m_position.x, m_position.y + 1.0f, m_position.z };
		up = { 0.0f, 0.0f, -1.0f };
		break;
	...
	case D3D11_TEXTURECUBE_FACE_POSITIVE_Z:
		target = { m_position.x, m_position.y, m_position.z + 1.0f };
		up = { 0.0f, 1.0f, 0.0f };
		break;
...
void EnvironmentMapper::SaveFace (const dx_ptr<ID3D11DeviceContext> & context, D3D11_TEXTURECUBE_FACE face) {
	...
	context->CopySubresourceRegion (m_envTexture.get (), static_cast<UINT>(face), 0, 0, 0, m_faceTexture.get (), 0, nullptr);
}