...
const XMFLOAT3 RoomDemoTextured::TEAPOT_POS{ -1.3f, -0.74f, -0.6f };
const XMFLOAT4 RoomDemoTextured::TABLE_POS{ 0.5f, -0.96f, 0.5f, 1.0f };
const XMFLOAT4 RoomDemoTextured::LIGHT_POS[2] = { {1.0f, 1.0f, 1.0f, 1.0f}, {-1.0f, -1.0f, -1.0f, 1.0f} };

RoomDemoTextured::RoomDemoTextured (HINSTANCE appInstance)
	...
	m_samplerState2 = m_device.CreateSamplerState (sd);
...
void RoomDemoTextured::UpdateCameraCB (XMMATRIX viewMtx) {
	XMVECTOR det;
...
void RoomDemoTextured::UpdateLamp (float dt) {
...
void mini::gk2::RoomDemoTextured::UpdateParticles (float dt) {
...
void RoomDemoTextured::Update (const Clock & c) {
...
void RoomDemoTextured::SetWorldMtx (DirectX::XMFLOAT4X4 mtx) {
	UpdateBuffer (m_cbWorldMtx, mtx);
}

void RoomDemoTextured::SetSurfaceColor (DirectX::XMFLOAT4 color) {
	UpdateBuffer (m_cbSurfaceColor, color);
}

void mini::gk2::RoomDemoTextured::SetShaders (const dx_ptr<ID3D11VertexShader> & vs, const dx_ptr<ID3D11PixelShader> & ps) {
	m_device.context ()->VSSetShader (vs.get (), nullptr, 0);
	m_device.context ()->PSSetShader (ps.get (), nullptr, 0);
}

void mini::gk2::RoomDemoTextured::SetTextures (std::initializer_list<ID3D11ShaderResourceView *> resList, const dx_ptr<ID3D11SamplerState> & sampler) {
...
void RoomDemoTextured::DrawMesh (const Mesh & m, DirectX::XMFLOAT4X4 worldMtx) {
	SetWorldMtx (worldMtx);
	m.Render (m_device.context ());
}

void RoomDemoTextured::DrawParticles () {
...
void RoomDemoTextured::DrawWalls () {
	//draw ceiling
	XMFLOAT4X4 texMtx;
	...
	XMStoreFloat4x4 (&texMtx, XMMatrixScaling (0.25f, 0.25f, 0.25f) * XMMatrixTranslation (0.5f, 0.5f, 0.0f) * XMMatrixScaling (1.0f, 16.0f, 1.0f));
	UpdateBuffer (m_cbTex1Mtx, texMtx);
...
void RoomDemoTextured::DrawTeapot () {
...
void RoomDemoTextured::DrawTableElement (Mesh & m, DirectX::XMFLOAT4X4 worldMtx) {
	SetWorldMtx (worldMtx);
...
void RoomDemoTextured::DrawTableLegs (XMVECTOR camVec) {
...
void RoomDemoTextured::DrawTransparentObjects () {
...
void RoomDemoTextured::DrawScene () {
	DrawWalls ();
	DrawTeapot ();
	SetShaders (m_phongVS, m_phongPS);
...
void RoomDemoTextured::Render () {
	Base::Render ();
	// TODO: 1.20 Change projection matrix to for drawing in environment cube
	auto projmtx = m_envMapper.FaceProjMtx ();
	UpdateBuffer (m_cbProjMtx, projmtx);
	...
	for (int i = 0; i <= 5; ++i) {
		...
		auto vwmtx = m_envMapper.FaceViewMtx (face);

		m_envMapper.ClearTarget (m_device.context ());

		UpdateCameraCB (vwmtx);
		DrawScene ();
