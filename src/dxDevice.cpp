...
dx_ptr<ID3D11HullShader> mini::DxDevice::CreateHullShader (std::vector<BYTE> hsCode) const {
	ID3D11HullShader * result = nullptr;
	auto hr = m_device->CreateHullShader (reinterpret_cast<const void *> (hsCode.data ()),
		hsCode.size (), nullptr, &result);

	dx_ptr<ID3D11HullShader> shader{ result };
	if (FAILED (hr)) {
		THROW_DX (hr);
	}
...
dx_ptr<ID3D11DomainShader> mini::DxDevice::CreateDomainShader (std::vector<BYTE> dsCode) const {
	ID3D11DomainShader * result = nullptr;
	auto hr = m_device->CreateDomainShader (reinterpret_cast<const void *> (dsCode.data ()),
		dsCode.size (), nullptr, &result);

	dx_ptr<ID3D11DomainShader> shader{ result };
	if (FAILED (hr)) {
		THROW_DX (hr);
	}
...
dx_ptr<ID3D11ShaderResourceView> mini::DxDevice::CreateShaderResourceView (const dx_ptr<ID3D11Texture2D> & texture,
