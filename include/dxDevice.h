...
namespace mini {
	class DxDevice {
		public:
			...
			const dx_ptr<ID3D11Device> & device () const { return m_device; }
			...
			dx_ptr<ID3D11HullShader> CreateHullShader (std::vector<BYTE> hsCode) const;
			dx_ptr<ID3D11DomainShader> CreateDomainShader (std::vector<BYTE> dsCode) const;
