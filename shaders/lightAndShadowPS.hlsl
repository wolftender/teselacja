...
float4 main(PSInput i) : SV_TARGET
{
	...
	float4 projTexCoord = mul(mapMtx, float4(i.worldPos, 1.0f));
	...
	if (i.worldPos.y < lightPos.y) {
		float4 sampledMapColor = lightMap.Sample (colorSampler, projTexCoord.xy);
		float c = max (sampledMapColor.r, defLightColor.r);

		lightColor = float4(c, c, c, c);

		float4 sampledShadow = shadowMap.Sample(colorSampler, projTexCoord.xy);
		float s = sampledShadow.r;
